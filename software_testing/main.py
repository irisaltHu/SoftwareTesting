...
def generate():
    generator = DataGenerator(data_path=original_data_path, out_path=generated_data_path, sample_per_folder=10)
    generator.generate()


# select mutations with the selector models. to add more model, use models.append_model
def select():
    models = Models(data_path=generated_data_path, label_path=label_path, out_path=output_path)
    models.append_model(selector_config_path, selector_checkpoint_path)
    selector = DataSelector(generated_data_path, label_path, selected_data_path, top_k=top_k, use_increment=False)
    selector.select(models)


# test on the target models. to add more model, use models.append_model
def model_test():
    models = Models(data_path=selected_data_path, label_path=label_path, out_path=output_path)
    models.append_model(target_config_path, target_checkpoint_path)
    models.traverse(top_k=top_k)
